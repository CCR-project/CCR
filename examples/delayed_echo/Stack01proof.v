...
Section SIMMODSEM.
  ...
  Proof.
    ...
    { unfold popF. init.
      ...
      iRefresh. do 5 iDestruct PRE. iMod A. iMod PRE. clarify.
      ...
      { instantiate (2:= (_, _, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitP; ss; et. }
      { esplits; ss; et. eauto with ord_step. }
      ss. iDestruct POST; subst. rewrite Any.upcast_downcast. steps. iRefresh. iDestruct POST. iMod A. apply Any.upcast_inj in A; des; clarify.
      ...
      - iMod A0. subst.
        ...
        { instantiate (2:= (_, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitR A; ss; et. right; iRefresh. rr. esplits; et. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST0; subst. rewrite Any.upcast_downcast. steps. iRefresh. iDestruct POST0. iMod A. apply Any.upcast_inj in A; des; clarify.
        clear POST0. steps.
      ...
      - do 4 iDestruct A0. iMod A0. subst.
        ...
        { instantiate (2:= (_, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitR A1; ss; et. do 4 left; do 3 eexists; iRefresh.
          iSplitP; ss. iSplitP; ss. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST0; subst. rewrite Any.upcast_downcast. steps. iRefresh. iDestruct POST0. iMod A1. apply Any.upcast_inj in A1; des; clarify.
        steps.

        acall_tac __ __ (@URA.unit Σ) (A0, A, POST) POST0; ss; et.
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. do 2 iDestruct POST1. iMod A1. apply Any.upcast_inj in A1. des; clarify. steps.


        acall_tac __ __ (@URA.unit Σ) (A, POST, POST1) A0; ss; et.
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. do 2 iDestruct POST0. iMod A0. apply Any.upcast_inj in A0. des; clarify. steps.
        rewrite Any.upcast_downcast in *. clarify.


        acall_tac __ __ (@URA.unit Σ) (A, POST, POST0) POST1; ss; et.
        { instantiate (2:=(_, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST2; subst. steps. rewrite Any.upcast_downcast in *. clarify.


        acall_tac __ __ (@URA.unit Σ) (A, POST) POST0; ss; et.
        { instantiate (2:=(_, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST1; subst. steps. rewrite Any.upcast_downcast in *. clarify.


        acall_tac __ __ (@URA.unit Σ) (A) POST; ss; et.
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST0; subst. steps. rewrite Any.upcast_downcast in *. clarify.

        astop. force_l. eexists.
        hret_tac (@URA.unit Σ) (POST0, A); ss.
        { split; ss. esplits; try refl; iRefresh. iSplitP; ss. eexists; iRefresh. iSplitR A; et. }
    }
    econs; ss.
    { unfold pop2F. init.
      harg_tac. des_ifs_safe. des. repeat rewrite URA.unit_idl in *. repeat rewrite URA.unit_id in *. ss.
      iRefresh. do 5 iDestruct PRE. iDestruct A0. iMod A. iMod PRE. clarify.
      apply Any.upcast_inj in PRE. des; clarify. steps. rewrite Any.upcast_downcast in *. clarify. steps.
      ...
      - do 4 iDestruct A1. iMod A1. subst.
        ...
        { instantiate (2:= (_, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitR A1; ss; et. do 4 left; do 3 eexists; iRefresh.
          iSplitP; ss. iSplitP; ss. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST; subst. rewrite Any.upcast_downcast. steps. iRefresh. iDestruct POST. iMod A1. apply Any.upcast_inj in A1; des; clarify.
        steps.

        acall_tac __ __ (@URA.unit Σ) (A0, A, A2) POST; ss; et.
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. do 2 iDestruct POST0; subst. iMod A1. apply Any.upcast_inj in A1. des; clarify. steps.


        acall_tac __ __ (@URA.unit Σ) (A, A0, POST0) A2; ss; et.
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. do 2 iDestruct POST; subst. iMod A1. apply Any.upcast_inj in A1. des; clarify. steps.
        rewrite Any.upcast_downcast in *. clarify.
        ...
        { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitP; ss. et. }
        { esplits; ss. eauto with ord_step. }
        ss. iDestruct POST; subst. steps. rewrite Any.upcast_downcast in *. clarify.

        astop. force_l. eexists.
        hret_tac (@URA.unit Σ) (POST, A); ss.
        { split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitR POST; ss. iSplitP; ss. }
    }
    econs; ss.
    { unfold pushF. init.
      harg_tac. des_ifs_safe. des. repeat rewrite URA.unit_idl in *. repeat rewrite URA.unit_id in *. ss.
      ...
      { esplits; ss. eauto with ord_step. }
      do 3 iDestruct POST; subst. iMod POST. rewrite Any.upcast_downcast. apply Any.upcast_inj in POST. des; clarify. steps.
      ...
      { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitR A; ss. et. }
      { esplits; ss. eauto with ord_step. }
      ss. iDestruct POST; subst. rewrite Any.upcast_downcast. steps.

      acall_tac __ __ (@URA.unit Σ) (A0, POST) A1; ss; et.
      { instantiate (2:=(_, _, _)). split; ss. esplits; try refl; iRefresh. eexists; iRefresh. iSplitP; ss. iSplitR A1; ss. et. }
      { esplits; ss. eauto with ord_step. }
      ss. iDestruct POST0; subst. rewrite Any.upcast_downcast. steps.

      astop. force_l. eexists.
      hret_tac (@URA.unit Σ) (A0, POST, POST0); ss.
      { esplits; try refl; iRefresh. split; ss. eexists; iRefresh. iSplitP; ss. do 2 eexists; iRefresh. iSplitR A0; et. iSplitP; ss; et.
