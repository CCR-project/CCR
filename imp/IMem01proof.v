Require Import IMem0 IMem1 HoareDef SimModSem.
...
Section SIMMODSEM.
  ...
  Context `{Σ: GRA.t}.
  Context `{@GRA.inG IMem1.memRA Σ}.

  Let W: Type := ((Σ * Any.t)) * ((Σ * Any.t)).
  (* Eval compute in (@RA.car (RA.excl Mem.t)). *)
  Eval compute in (@URA.car IMem1._memRA).
  ...
  Let wf: W -> Prop :=
    fun '(mrps_src0, mrps_tgt0) =>
      exists mr_src (mem_tgt: Mem.t),
        ...
        (<<SIM: iHyp (Exists mem_src, Own (GRA.embed ((Auth.black mem_src): URA.car (t:=IMem1.memRA))) **
  ...
  Theorem correct: ModPair.sim IMem1.Mem IMem0.IMem.
  Proof.
    ...
    { unfold allocF. init.
      ...
      iRefresh. iPure PRE. des. clarify.
      Admitted.

  (*     apply Any.upcast_inj in PRE. des; clarify. *)
  ...
  (*     assert(WF': URA.wf (mem_src1: URA.car (t:=IMem1._memRA))). *)
  ...
  (*       Local Opaque IMem1._memRA. *)
  (*       ss. destruct H; clear H. (*** coq bug; des infloops ***) des. clarify. *)
  (*       esplits; et. *)
  (*       Local Transparent IMem1._memRA. *)
  (*       unfold mem_src1. ss. *)
  ...
  (*     assert (VALIDPTR: forall b ofs v (WF: URA.wf ((Auth.black (mem_src0: URA.car (t:=IMem1._memRA))) ⋅ ((b, ofs) |-> [v]))), *)
