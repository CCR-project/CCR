...
Section SIM.
  ...
  | hsim_apc_step
      ...
      (POST: exists fuel1,
          ...
          (<<SIM: forall fmr1 st_src1 st_tgt1 ret_src ret_tgt x_tgt
                         (ACC: current_iProp fmr1 (FR ** inv_with w0 st_src1 st_tgt1 **
                                                      (fsp_src.(postcond) (Some mn) x_src ret_src ret_tgt) **
                                                      (fsp_tgt.(postcond) (Some mn) x_tgt ret_tgt ret_tgt))),
  ...
  | hsim_apc_both
      ...
      fuel f_src f_tgt
      (SIM: _hsim hsim Q fmr (fuel1) true true (st_src, ktr_src tt) (st_tgt, ktr_tgt tt))
    :
      _hsim hsim Q fmr fuel f_src f_tgt (st_src, trigger (hAPC) >>= ktr_src) (st_tgt, trigger hAPC >>= ktr_tgt)
  | hsim_syscall
      fn arg rvs
      st_src st_tgt ktr_src ktr_tgt
      fuel f_src f_tgt
  ...
  Lemma _hsim_ind2
        ...
        (APCBOTH: forall
            ...
            fuel f_src f_tgt
            (SIM: _hsim hsim Q fmr (fuel1) true true (st_src, ktr_src tt) (st_tgt, ktr_tgt tt))
            (IH: P (fuel1) true true (st_src, ktr_src tt) (st_tgt, ktr_tgt tt)),
  ...
  Lemma hsim_ind
        ...
        (APCBOTH: forall
            ...
            st_src st_tgt ktr_src ktr_tgt
            fuel f_src f_tgt
            (SIM: hsim Q fmr (fuel1) true true (st_src, ktr_src tt) (st_tgt, ktr_tgt tt))
            (IH: P (fuel1) true true (st_src, ktr_src tt) (st_tgt, ktr_tgt tt)),
            P fuel f_src f_tgt (st_src, trigger (hAPC) >>= ktr_src) (st_tgt, trigger hAPC >>= ktr_tgt))
        (SYSCALL: forall
            fn arg rvs
  ...
  Definition mylift (fuel: option Ord.t) (mn_caller: option mname) X (x: X)
    ...
    match fuel with
    | None =>
      (interp_hCallE_tgt mn stb_src o (interp_hEs_tgt itr_src) fr) >>= (HoareFunRet Q mn_caller x)
    | Some fuel =>
      r0 <- (interp_hCallE_tgt mn stb_src o (_APC fuel) fr);;
      r1 <- (interp_hCallE_tgt mn stb_src o (tau;; Ret (snd r0)) (fst r0));;
      r2 <- (interp_hCallE_tgt mn stb_src o (interp_hEs_tgt itr_src) (fst r1));;
  ...
  Lemma hsim_adequacy_aux `{PreOrder _ le}:
    forall
      f_src f_tgt st_src st_tgt (itr_src: itree (hAPCE +' Es) Any.t) itr_tgt (mr_src: Σ) fr_src fr_tgt
      X (x: X) Q mn_caller fuel w0
      (SIM: hsim (fun st_src st_tgt ret_src ret_tgt =>
                    (inv_with w0 st_src st_tgt) ** (Q mn_caller x ret_src ret_tgt: iProp)) (fr_src ⋅ mr_src)
      ...
      paco8 (_sim_itree (mk_wf2 I) le) bot8 Any.t Any.t (lift_rel (mk_wf2 I) le w0 (@eq Any.t))
            ...
            (Any.pair st_src mr_src↑,
             mylift fuel mn_caller x fr_src Q itr_src)
            (st_tgt, '(_, r) <- interp_hCallE_tgt mn stb_tgt o (interp_hEs_tgt itr_tgt) fr_tgt;; Ret r).
  Proof.
    ginit. gcofix CIH. i.
