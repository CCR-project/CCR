...
Module OrderSort (A: TotalOrderBool).
  ...
  Definition min (a0 a1: A.t): A.t :=
    if (A.leb a0 a1)
    then a0
    else a1
  .

  Global Program Instance leb_PreOrder: Reflexive A.leb.
  Next Obligation.
    destruct (A.leb_total x x); ss.
  Qed.

  Global Program Instance eqA_PreOrder: PreOrder A.eqA.
  Next Obligation.
    ii. eapply A.eqb_eq; et.
    destruct (A.leb_total x x); ss.
  Qed.
  Next Obligation.
    ii. eapply A.eqb_eq; et. eapply A.eqb_eq in H; et. eapply A.eqb_eq in H0; et. des.
    esplits; et.
    - eapply A.leb_trans; et.
    - eapply A.leb_trans; et.
  Qed.

  Global Program Instance eqA_Symmetric: Symmetric A.eqA.
  Next Obligation.
    eapply A.eqb_eq in H. eapply A.eqb_eq. des; et.
  Qed.

  (* Lemma min_comm: forall a b (NEQ: ~A.eqA a b), A.eqA (min a b) (min b a). *)
  (* Proof. *)
  (*   i. unfold min. des_ifs; try refl. *)
  (*   - eapply A.eqb_eq; et. *)
  (*   - destruct (A.leb_total a b); ss; clarify. *)
  (* Qed. *)
  
  Ltac des_nodup :=
    repeat match goal with
           | [H: NoDupA _ (_ :: _) |- _ ] => inv H
           | [H: Forall _ (_ :: _) |- _ ] => inv H
           end.

  Lemma min_comm: forall a b (NODUP: NoDupA A.eqA [a; b]), (min a b) = (min b a).
  (* Lemma min_comm: forall a b (NEQ: ~A.eqA a b), (min a b) = (min b a). *)
  Proof.
    i. des_nodup. unfold min. des_ifs; try refl.
    - contradict H1. eapply A.eqb_eq; et.
    - destruct (A.leb_total a b); ss; clarify.
  Qed.

  Lemma min_assoc: forall a b c (NODUP: NoDupA A.eqA [a;b;c]), (min a (min b c)) = (min (min a b) c).
  Proof.
    i. unfold min. des_ifs; try refl.
    - exfalso.
      destruct (A.leb_total a c); ss; clarify.
      assert(A.leb a c).
      { eapply A.leb_trans; et. }
      assert(A.eqA a c).
      { eapply A.eqb_eq; et. }
    ...
    - exfalso.
      destruct (A.leb_total a b); ss; clarify.
      destruct (A.leb_total b c); ss; clarify.
      assert(A.leb c a).
      { eapply A.leb_trans; et. }
      assert(A.eqA a c).
      { eapply A.eqb_eq; et. }
  ...
  (* Definition list_min (l: list A.t) (d: A.t) := fold_right min d l. *)
  Definition list_min (l: list A.t): option (A.t) :=
    match l with
    | [] => None
    | hd :: tl => Some (fold_right min hd tl)
    end
  .

  (* Instance Permutation_list_min: Proper (@Permutation A.t ==> eq) (list_min) | 10. *)
  (* Proof. *)
  ...
  Lemma fold_right_min_nodup
        xs ys y
        (NODUP: NoDupA A.eqA (xs ++ ys))
        (MIN: list_min ys = Some y)
    :
      NoDupA A.eqA (xs ++ [y])
  .
  Proof.
  ...
  Ltac nodup_solver := des_nodup; repeat (econs; ss; et); by (let T := fresh "T" in intro T; sym in T; ss).

  Lemma fold_right_min_comm
        a b l0
        (NODUP: NoDupA A.eqA (a :: b :: l0))
    :
      (min a (fold_right min b l0)) = (min b (fold_right min a l0))
  .
  Proof.
    ginduction l0; ii; ss.
    - rewrite min_comm; ss.
    -
      rewrite min_assoc; ss.
      rewrite (@min_comm a0 a); ss.
      rewrite <- min_assoc; ss.

      rewrite (@min_assoc b); ss.
      rewrite (@min_comm b a); ss.
      rewrite <- min_assoc; ss.
      f_equal.
      ...
      { assert(U: NoDupA A.eqA ([a;b] ++ (a0::l0))).
        { nodup_solver. }
        eapply fold_right_min_nodup in U; ss; et. }
      { nodup_solver. }
      { assert(U: NoDupA A.eqA ([b;a] ++ (a0::l0))).
        { nodup_solver. }
        eapply fold_right_min_nodup in U; ss; et. }
      { assert(U: NoDupA A.eqA ([a;a0] ++ (b::l0))).
        { nodup_solver. }
        eapply fold_right_min_nodup in U; ss; et. }
      { nodup_solver. }
      { assert(U: NoDupA A.eqA ([a0;a] ++ (b::l0))).
        { nodup_solver. }
        eapply fold_right_min_nodup in U; ss; et. }
  Qed.

  Lemma list_min_cons_eq
        x l0 l1
        (NODUP0: NoDupA A.eqA (x :: l0))
        (NODUP1: NoDupA A.eqA (x :: l1))
        (EQ: list_min l0 = list_min l1)
    :
      list_min (x :: l0) = list_min (x :: l1)
  .
  Proof.
    destruct l0; ii; ss; clarify.
    - destruct l1; ss.
    - destruct l1; ss. f_equal.
      rewrite fold_right_min_comm at 1; ss; cycle 1.
      { nodup_solver. }
      rewrite (@fold_right_min_comm t0); ss; cycle 1.
      { nodup_solver. }
      clarify. rewrite H0. ss.
  Qed.

  Lemma unfold_list_min
        x xs
    :
      (list_min (x :: xs)) = match (list_min xs) with
                             | Some m => Some (min x m)
                             | _ => Some (x)
                             end
  .
  ...
  Lemma perm_list_min
        ...
        (PERM: Permutation l0 l1)
    :
      list_min l0 = list_min l1
  .
  Proof.
    exploit NoDupA_permutation; et. intro NODU.
    ginduction PERM; ii; ss.
    - exploit IHPERM; et. { inv NODUP; ss. } { inv NODU; ss. } intro T; clarify.
      eapply list_min_cons_eq; ss.
    - cbn.
      {
  Admitted.
  ...
  Lemma sorted_min
        hd tl
        (SORT: StronglySorted A.leb (hd :: tl))
        (NODUP: NoDupA A.eqA (hd :: tl))
    :
      <<MIN: list_min (hd :: tl) = Some hd>>
  .
  Proof.
  (*   ginduction tl; ii; ss. *)
  (*   r. f_equal. *)
  (*   eapply StronglySorted_inv in SORT. des. *)
  (*   exploit (@IHtl hd); et. *)
  (*   { eapply StronglySorted_inv in SORT. des. *)
  (*     econs; ss; et. inv SORT0; ss. } *)
  (*   { inv NODUP. inv TL. econs; ss. inv HD; ss. } *)
  (*   i; des; clarify. rewrite H0. *)
  (*   unfold min. inv SORT0. des_ifs. *)
  (*   exfalso. clear - NODUP H2 Heq. inv NODUP. inv HD. eapply H1. eapply A.eqb_eq; et. *)
  (* Qed. *)
  Admitted.
  ...
  Lemma permutation_sorted_unique (l0 l1: list A.t)
        ...
        (SORTED1: StronglySorted A.leb l1)
    :
      forall n, (nth_error l0 n) = (nth_error l1 n).
  Proof.
    i. pattern n. eapply lt_wf_ind. clear n. intros n IH.
    intro.
  Qed.
