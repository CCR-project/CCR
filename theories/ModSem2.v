...
Section EVENTS.
  ...
  .
  Proof.
    unfold interp_Es, interp_rE, interp_pE.
    irw.
    unfold resum, ReSum_id, id_, Id_IFun.
    grind. rewrite interp_state_vis. irw. grind.
    unfold resum, ReSum_id, id_, Id_IFun.
    destruct rst0.
    destruct e; ss; des_ifs; ss;
      repeat (try unfold assume, guarantee, triggerNB, resum, ReSum_id, id_, Id_IFun, pure_state; irw; try rewrite interp_state_vis; ired; grind; cbn).
    - admit "tau".
    - admit "tau".
    - admit "tau".
  Qed.

  Definition Es' := callE +' pE +' rE +' eventE.
  Definition interp_Es' A (prog: callE ~> itree Es') (itr0: itree Es' A) (st0: r_state * p_state): itree eventE ((r_state * p_state) * _)%type :=
    let (rst0, pst0) := st0 in
    '(rst1, (pst1, v)) <- interp_rE (interp_pE (interp_mrec prog itr0) pst0) rst0;;
    Ret ((rst1, pst1), v)
  .
    ...
    fun _ e '(mrs, frs) =>
      ...
      | hd :: tl =>
        ...
        | Discard fr =>
          ...
          (* Ret ((mrs, rest :: tl), tt) *)
          triggerUB
        | CheckWf mn => (* assume(URA.wf (URA.add (mrs mn) hd));; *)
          Ret ((mrs, frs), tt)
        | PushFrame =>
          Ret ((mrs, ε :: frs), tt)
        | PopFrame =>
  ...
  Proof.
    ...
    destruct rst0.
    destruct e; ss; des_ifs; ss;
      repeat (try unfold assume, guarantee, triggerUB, triggerNB, resum, ReSum_id, id_, Id_IFun, pure_state; irw; try rewrite interp_state_vis; ired; grind; cbn).
  Qed.
