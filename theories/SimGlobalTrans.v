...
(* Lemma simg_mon_ord r R RR o0 o1 (ORD: Ord.le o0 o1) (itr_src itr_tgt: itree eventE R): *)
...
(* Lemma simg_mon_all r r' S SS SS' o o' (LEr: r <5= r') (LEss: SS <2= SS') (LEo: Ord.le o o'): *)
...
| bindR_intro
  ...
  :
    (* bindR r s (OrdArith.add o0 o1) (ITree.bind i_src k_src) (ITree.bind i_tgt k_tgt) *)
...
(* Hint Resolve OrdArith.add_base_r: ord. *)
(* Hint Resolve OrdArith.add_base_l: ord. *)
(* Hint Resolve Ord.lt_le_lt: ord. *)
(* Hint Resolve Ord.le_lt_lt: ord. *)
...
Proof.
  r in LEFT. des.
  exploit (@choice { r0r1 : (R * R) | RR (fst r0r1) (snd r0r1) } Ord.t
  ...
  r. unshelve eexists (OrdArith.add (Ord.join f) o0).
  ...
  {
    unshelve eapply Ord.join_upperbound. eexists (_, _). ss. eauto.
  }
  gfinal. right.  ss.
Qed.
...
Lemma ordinal_ind2
      (P: Ord.t -> Prop)
      (IH: forall o0 (IH: forall o1, Ord.lt o1 o0 -> P o1), P o0)
  :
    forall o0, P o0
.
Proof.
  revert IH. eapply well_founded_induction. { eapply Ord.lt_well_founded. }
Qed.
...
Theorem simg_trans_gil
        ...
        (LE: Ord.le (myadd o0 o1) o2)
  :
    <<SIM: simg (rel_compose RR0 RR1) o2 i0 i2>>
.
Proof.
  revert_until RR1. pcofix CIH.
  eapply (ordinal_ind2 (fun o0 => forall o1 o2 i0 i1 i2 (SIM0: simg RR0 o0 i0 i1) (SIM1: simg RR1 o1 i1 i2) (LE: Ord.le (myadd o0 o1) o2), paco5 _simg r R (rel_compose RR0 RR1) o2 i0 i2)).
  ...
  - (** ret **)
    ...
    + pclearbot. pfold. econs; eauto.
      { instantiate (1:=i1). eapply (@Ord.lt_le_lt o0); auto.
    ...
    + des. pfold. econs; eauto.
      { instantiate (1:=(myadd i1 Ord.O)).
        eapply Ord.lt_le_lt; et.
        eapply Ord.lt_le_lt.
        - eapply myadd_lt_l; et.
        - eapply myadd_le_r. eapply Ord.O_is_O. }
  ...
  - (*** tau both ***)
    ...
    { rename o0 into ox. rename i1 into _ox. rename i3 into oy. rename o1 into oy'.
      ...
      { instantiate (1:=myadd _ox oy'). eapply Ord.lt_le_lt; et. apply myadd_lt_l; ss. }
      des. pclearbot.
      { right. eapply CIH; et. { pfold. econs; et. } refl. }
    }
    { pclearbot. pfold. econs; eauto; cycle 1.
      { right. eapply CIH; et. refl. }
      eapply Ord.lt_le_lt; et. eapply Ord.le_lt_lt.
  ...
  - (** tauL **)
    ...
    { pfold. econs; eauto.
      { instantiate (1:=myadd i1 i3).
        eapply (@Ord.lt_le_lt (myadd o0 o1)); auto.
        eapply (@Ord.lt_le_lt (myadd i1 o1)).
    ...
    { pclearbot. eapply IH; eauto.
      ...
      { eapply myadd_le_r. apply Ord.lt_le. auto. }
      { eapply myadd_le_l. apply Ord.lt_le. auto. }
    }
    { des. pclearbot.
      pfold. econs; et; cycle 1.
      { esplits; et. right. eapply CIH; eauto.
        ...
        reflexivity. }
      { eapply Ord.lt_le_lt; et. eapply myadd_lt_l; et. }
    }
    { pfold. econs; et; cycle 1.
  ...
  - (** tauR **)
    pclearbot.
    pfold. econs; eauto.
    { instantiate (1:= myadd o0 i3). eapply (Ord.lt_le_lt); et. eapply myadd_lt_r; et. }
    right. eapply CIH; et. refl.
  ...
  - (** chooseL **)
    ...
    { irw in H1. csc. assert(ktr_tgt0 = ktr_src0) by ss; subst; clear_tac.
      ...
      { eapply Ord.lt_le_lt; et.
        eapply Ord.le_lt_lt; et; cycle 1.
    ...
    { irw in H1. csc. assert(ktr_tgt0 = ktr_src0) by ss; subst; clear_tac.
      ...
      { etrans; et.
        etrans.
        { eapply myadd_le_r; et. eapply Ord.lt_le; et. }
        { eapply myadd_le_l; et. eapply Ord.lt_le; et. }
      }
    }
    { pclearbot.
      pfold. econs; eauto; cycle 1.
...
.
Proof.
  rr. exists (Ord.S Ord.O). pfold. econs; et. { apply Ord.S_lt. } left.
...
Proof.
  ...
  { replace (trigger (Choose nat)) with ((trigger (Choose nat)) >>= (fun n => Ret n)); cycle 1.
    { irw; ss. }
    r. exists (Ord.S Ord.O). pfold. econs; et. apply Ord.S_lt. }
  i. induction r1; ii; ss.
  { r. exists (Ord.S Ord.O). pfold. econs; et. }
  { etrans; et. r. exists (Ord.S Ord.O). pfold. econs; et. { apply Ord.S_lt. } left.
    eapply simg_paco_refl; ss. }
Qed.
...
Proof.
  ...
  { r. exists (Ord.S Ord.O).
    replace (trigger (Choose nat)) with ((trigger (Choose nat)) >>= (fun n => Ret n)); cycle 1.
    { irw; ss. }
    pfold. econs; et. apply Ord.S_lt.
  }
  i. subst.
  replace (Ret 1) with ((Ret 1: itree eventE _) >>= (fun x => Ret x)); cycle 1.
  { irw. ss. }
  ...
  apply eutt_bind with (RR:=fun n m => n = 1 /\ m = 1).
  {
    ...
    { irw. ss. }
    eapply eutt_bind with (RR:=top2).
    { r. exists (Ord.from_nat r2).
      induction r2; ii; ss.
      - pfold. econs; et.
      - pfold. econs; et. apply Ord.S_lt.
    }
    i. r.
    exists (Ord.S Ord.O). irw. pfold. econs; et.
  }
  i. des; subst.
  r. exists (Ord.S Ord.O). pfold. econs; et.
Qed.
...
Proof.
  r. exists (Ord.from_nat 5). pfold. econs; et.
  { refl. }
  i. esplits; et. left. pfold. econs; et. { eapply OrdArith.lt_from_nat. ss. } esplits; et. left.
  eapply simg_paco_refl; et.
Qed.
...
Proof.
  ...
  pfold. econs; eauto. { eapply OrdArith.lt_from_nat. ss. }
  i. left. pfold. econs; eauto. { eapply OrdArith.lt_from_nat. ss. }
