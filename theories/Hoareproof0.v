...
Set Implicit Arguments.
  ...
  Lemma simg_l_interp_Es_eventE
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 (interp_Es prog (trigger ((|e): (pE +' eventE) R)%sum) st0) x5>>
  .
  Proof. cbn. rewrite interp_Es_eventE. ss. Qed.
  ...
  Lemma simg_l_interp_Es_pE
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 (interp_Es prog (trigger ((e|): (pE +' eventE) R)%sum) (rst0, pst0)) x5>>
  .
  Proof. cbn. rewrite interp_Es_pE. ss. Qed.
  ...
  Lemma simg_l_interp_Es_eventE2
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 ((interp_Es prog (trigger ((|e): (pE +' eventE) R)%sum) st0) >>= h) x5>>
  .
  Proof. cbn. rewrite interp_Es_eventE. rp; et. grind. Qed.
  ...
  Lemma simg_l_interp_Es_pE2
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 ((interp_Es prog (trigger ((e|): (pE +' eventE) R)%sum) (rst0, pst0)) >>= h) x5>>
  .
  Proof. cbn. rewrite interp_Es_pE. rp; et. grind. Qed.
  ...
  Lemma simg_r_interp_Es_eventE
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 x5 (interp_Es prog (trigger ((|e): (pE +' eventE) R)%sum) st0)>>
  .
  Proof. cbn. rewrite interp_Es_eventE. ss. Qed.
  ...
  Lemma simg_r_interp_Es_pE
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 x5 (interp_Es prog (trigger ((e|): (pE +' eventE) R)%sum) (rst0, pst0))>>
  .
  Proof. cbn. rewrite interp_Es_pE. ss. Qed.
  ...
  Lemma simg_r_interp_Es_eventE2
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 x5 ((interp_Es prog (trigger ((|e): (pE +' eventE) R)%sum) st0) >>= h)>>
  .
  Proof. cbn. rewrite interp_Es_eventE. rp; et. grind. Qed.
  ...
  Lemma simg_r_interp_Es_pE2
    ...
    :
      <<SIM: gpaco5 _simg x0 x1 x2 _ x3 x4 x5 ((interp_Es prog (trigger ((e|): (pE +' eventE) R)%sum) (rst0, pst0)) >>= h)>>
  .
  Proof. cbn. rewrite interp_Es_pE. rp; et. grind. Qed.
...
Section CANCEL.
  ...
  Proof.
    ...
    { steps. esplits; eauto. steps. esplits; eauto. steps. unfold unwrapU.
      ...
      { steps.
        apply simg_l_interp_Es_bind2.
        match goal with
    | [ |- (gpaco5 _simg _ _ _ _ _ _ ((interp_Es _ (trigger ?e) _) >>= _) _) ] =>
     idtac e
        end.
        TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
        apply simg_l_interp_Es_eventE.
        match goal with
    | [ |- (gpaco5 _simg _ _ _ _ _ _ ((interp_Es _ (trigger ?e) _) >>= _) _) ] =>
         multimatch type of e with
         | context [ rE ] => idtac "A" (* apply simg_l_interp_Es_rE *)
         | context [ eventE ] => apply simg_l_interp_Es_eventE
         | context [ pE ] => idtac "C" (* apply simg_l_interp_Es_pE *)
         | context [ callE ] => idtac "D" (* apply simg_l_interp_Es_callE *)
         | _ => fail 2
         end
        end.
        ired_l.
