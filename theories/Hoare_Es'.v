...
Section PROOF.
  ...
  (*            (P: X -> Y -> Any_tgt -> Σ -> ord -> Prop) *)
  (*            (Q: X -> Z -> Any_tgt -> Σ -> Prop) *)
  (*            (body: ord -> Y -> itree Es' Z): Any_tgt -> itree Es' Any_tgt := let _ := mn in admit "". *)





  (******** Es' does not work, we need rE **************)
  (* Definition HoareFun *)
  (*            (P: X -> Y -> Any_tgt -> Σ -> ord -> Prop) *)
  (*            (Q: X -> Z -> Any_tgt -> Σ -> Prop) *)
  (*            (body: ord -> Y -> itree (rE +' Es') Z): Any_tgt -> itree (rE +' Es') Any_tgt := fun varg_tgt => *)
  (*   varg_src <- trigger (Take Y);; *)
  (*   x <- trigger (Take X);; *)
  (*   rarg <- trigger (Take Σ);; forge rarg;; (*** virtual resource passing ***) *)
  (*   (checkWf mn);; *)
 ...
 ***)
  ...
  Definition body_to_tgt {AA AR} (mn: mname) (cur: ord)
  ...
  Definition HoareFun
    ...
    vret_src <- ((body_to_tgt mn ord_cur body) varg_src);; (*** "rudiment": we don't remove extcalls because of termination-sensitivity ***)
...
***)
  ...
  Proof.
    ...
    unfold unwrapN at 1. des_ifs; cycle 1.
    { steps. }
...
Notation "(⋅)" := URA.add (only parsing).
      ...
      steps. esplits; eauto. steps. unfold cfun. steps. rewrite CAST. cbn. steps.
      (* esplits; eauto. *)
      ...
      + guclo ordC_spec. econs; eauto. { instantiate (1:=Ordinal.from_nat 100). eapply from_nat_le; ss; try lia. }
        ...
        { instantiate (2:=(fsb_body f a) >>= (fun r => Ret r↑)).
        ...
        admit "ez -- updatable".
      + bar. ii. des_ifs. des; subst. unfold unwrapN. des_ifs; cycle 1.
        { steps. }
        steps. unfold put. steps. unfold handle_rE. des_ifs. { steps. } steps.
        unfold guarantee. steps. unfold discard. steps. unfold guarantee. steps.
        ss. des. esplits; ss; et.
        clear - WFTGT0 x3.
        admit "ez -- updtaable".
...
(x0 <- interp_Es (ModSem.prog ms_tgt)
                 (interp_hCallE_tgt (E:=pE +' eventE) stb "Main" ord_top (trigger (hCall "main" ([]: list val)↑))) st_tgt0;; Ret (snd x0))
(x0 <- interp_Es (ModSem.prog ms_tgt)
...
we should know that stackframe is not popped (unary property)". }
    ...
    replace (x0 <- interp_Es (ModSem.prog ms_tgt) ((ModSem.prog ms_tgt) _ (Call "main" ([]: list val)↑)) st_tgt0;;
             Ret (snd x0)) with
        (x0 <- interp_Es (ModSem.prog ms_tgt) (interp_hCallE_tgt (E:=pE +' eventE) stb "Main" ord_top (trigger (hCall "main" ([]: list val)↑)))
