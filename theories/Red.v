...
Ltac red_all tac success :=
  ...
  tryif (let f := fresh "f" in
         ...
         [tac f|
          ...
          | [f0:= ?f1: _flag|- _] =>
            match f1 with
            | _continue => subst f; red_all tac true
...
(* Ltac _foo f := *)
(*   ((instantiate (f:=_continue); apply foo0; fail) || *)
(*    (instantiate (f:=_continue); apply foo1; fail) || *)
(*    (instantiate (f:=_break); apply foo2; fail)). *)

(* Goal f 6. *)
(* Proof. *)
(*   pred foopattern _foo. *)


(* itree lemma *)

Lemma trigger_ret_rev (E : Type -> Type) (R V : Type) (e : E V)
  :
    trigger e = trigger e >>= (fun x => Ret x).
Proof.
  symmetry. apply bind_ret_r.
Qed.
...
Ltac _red_itree f :=
  ...
  | [ |- ITree.bind' _ (trigger _) = _] =>
    instantiate (f:=_break); apply trigger_ret_rev; fail 2
  | _ =>
    instantiate (f:=_fail 2)
  end.


(* interp_tgt reduction *)
...
Ltac _red_interp_tgt f :=
  ...
  | [ |- ITree.bind' _ (interp_hCallE_tgt _ _ _ (trigger _)) = _ ] =>
    ...
    (apply interp_tgt_hcall ||
     ...
     apply interp_tgt_triggerp); fail
...
Ltac red_lsim_l := pred lsim_l_context _red_lsim.
Ltac red_lsim_r := pred lsim_r_context _red_lsim.

Ltac red_lsim := red_lsim_l || red_lsim_r.
...
Ltac force_l :=
  ...
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, guarantee ?P >>= _) (_, _)) ] =>
    ...
    destruct (classic P) as [name|name]; [red_lsim; gstep; eapply sim_itree_choose_src; [eauto with ord_step|exists name]|contradict name]; cycle 1
  ...
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, ITree.bind' _ (interp _ (guarantee ?P ))) (_, _)) ] =>
    ...
    remember (guarantee P) as tvar eqn:thyp; unfold guarantee in thyp; subst tvar;
    let name := fresh "_GUARANTEE" in
    destruct (classic P) as [name|name]; [red_lsim; gstep; eapply sim_itree_choose_src; [eauto with ord_step|exists name]|contradict name]; cycle 1; clear name
...
Ltac force_r :=
  ...
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, _) (_, assume ?P >>= _)) ] =>
    ...
    destruct (classic P) as [name|name]; [red_lsim; gstep; eapply sim_itree_take_tgt; [eauto with ord_step|exists name]|contradict name]; cycle 1
...
Ltac _step :=
  ...
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, unwrapU ?ox >>= _) (_, _)) ] =>
    ...
    remember (unwrapU ox) as tvar eqn:thyp; unfold unwrapU in thyp; subst tvar;
    let name := fresh "_UNWRAPU" in
    destruct (ox) eqn:name; [|unfold triggerUB; red_lsim; force_l; ss; fail]
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, assume ?P >>= _) (_, _)) ] =>
    let tvar := fresh "tmp" in
    let thyp := fresh "TMP" in
    remember (assume P) as tvar eqn:thyp; unfold assume in thyp; subst tvar;
    let name := fresh "_ASSUME" in
    red_lsim; gstep; eapply sim_itree_take_src; [eapply OrdArith.lt_from_nat; apply Nat.lt_succ_diag_r|]; intro name
  ...
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, _) (_, unwrapN ?ox >>= _)) ] =>
    ...
    remember (unwrapN ox) as tvar eqn:thyp; unfold unwrapN in thyp; subst tvar;
    let name := fresh "_UNWRAPN" in
    destruct (ox) eqn:name; [|unfold triggerNB; red_lsim; force_r; ss; fail]
  | [ |- (gpaco6 (_sim_itree _) _ _ _ _ _ _ _ (_, _) (_, guarantee ?P >>= _)) ] =>
    let tvar := fresh "tmp" in
    let thyp := fresh "TMP" in
    remember (guarantee P) as tvar eqn:thyp; unfold guarantee in thyp; subst tvar;
    let name := fresh "_GUARANTEE" in
    red_lsim; gstep; eapply sim_itree_choose_tgt; [eapply OrdArith.lt_from_nat; apply Nat.lt_succ_diag_r|]; intro name
...
.
Ltac steps := repeat ((*** pre processing ***) (try red_lsim); try _step; (*** post processing ***) unfold alist_add; simpl; des_ifs_safe).
...
Section HLEMMAS.
  ...
  Proof.
    ...
    { eapply OrdArith.lt_add_r. eauto with ord_step. }
    exists (fn, args↑).
    gstep. eapply sim_itree_tau_src.
...
Ltac _harg_tac := red_lsim; eapply harg_clo; i.

Ltac _hcall_tac x o mr_src1 fr_src1 rarg_src := red_lsim; eapply (@hcall_clo _ _ x o mr_src1 fr_src1 rarg_src); [|eapply OrdArith.lt_from_nat; lia|..].

Ltac _hret_tac mr_src1 rret_src := red_lsim; eapply (@hret_clo _ mr_src1 rret_src); [eapply OrdArith.lt_from_nat; lia|..].
